...
export const trackOtpAttempts = async(email:string,otp:Number)=>{
    ...
    if(storedOtp != otp){
        if(otpAttemptCount >= 3){
            ...
            throw new BadRequestError("Multiple failed attempts! please try again after 30minutes.");
        }
        await redis.set(`otp-attempts:${email}`,otpAttemptCount+1,"EX",1800);
        throw new BadRequestError(`Incorrect OTP! You have ${3-otpAttemptCount} attempts left`);
    }
...
export const validateOtp = async(email:string)=>{
    ...
    {
        throw new BadRequestError("Too many otp requeests! Please wait for 1hour to generate new OTP");
    }
    if(await redis.get(`otp-lock:${email}`))
    {
        throw new BadRequestError("Multiple failed attempts! Please wait for 30 minutes for requesting new OTP");
    }
    if(await redis.get(`otp-cooldown:${email}`)){
        throw new BadRequestError("Please wait for 1min to request for new Otp");
    }
    return;
}
